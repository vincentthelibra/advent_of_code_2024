...
def part_2(df_list_a, df_list_b):
    frequency_df_a = df_list_b[df_list_b.isin(df_list_a)].value_counts().reset_index()

    frequency_df_a.columns = ["Element", "Frequency"]

    frequency_df_a["Similarity_Score"] = (
        frequency_df_a["Element"] * frequency_df_a["Frequency"]
    )
